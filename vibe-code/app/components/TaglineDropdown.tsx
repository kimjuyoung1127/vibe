...
const TaglineDropdown: React.FC<TaglineDropdownProps> = ({
  ...
  useEffect(() => {
    if (searchTerm === '') {
      ...
      const filtered = projectTaglines.filter(tagline =>
        t(tagline.text).toLowerCase().includes(term) ||
  ...
  const handleTaglineSelect = (tagline: ProjectTagline) => {
    const translatedText = t(tagline.text);
  ...
  const getCategoryLabel = (category: string): string => {
    return t(`projects.categories.${category}` as any) || category;
  };
    ...
    <div className="flex flex-col gap-1 w-full" ref={dropdownRef}>
      ...
      <div className="relative">
        <input
          ...
          placeholder={placeholder || t('projects.coreInfo.taglinePlaceholder', 'Select or enter a one-line summary (Tagline)')}
