...
const ToolTechReviewForm = () => {
  ...
  useEffect(() => {
    const loadReviewData = async () => {
      ...
      try {
        ...
        setErrors({ general: err.message || t('common.failedToLoadReviewData', 'Failed to load review data. Please try again.') });
      }
    };
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      if (!formData.title.trim()) {
        setErrors({ title: t('common.titleIsRequiredToSaveDraft', 'Title is required to save a draft.') });
      ...
      const { data: { session } } = await supabase.auth.getSession();
      if (!session?.user) {
        setErrors({ general: t('common.needToBeLoggedInToSaveDraft', 'You need to be logged in to save a draft.') });
        setSaveDraftStatus('error');
        return;
      }
      ...
      if (isEditingDraft && draftId) {
        ...
        if (!newReviewId) throw new Error(t('common.failedToGetNewReviewId', 'Failed to get the ID of the newly created draft.'));
      }
      ...
      setErrors({ general: error.message || t('common.failedToSaveDraft', 'Failed to save draft. Please try again.') });
      setSaveDraftStatus('error');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!formData.title.trim()) {
        setErrors({ title: t('common.titleIsRequired', 'Title is required.') });
        setIsSubmitting(false);
        return;
      }

      if (!formData.toolTechName.trim()) {
        setErrors({ toolTechName: t('common.toolTechNameIsRequired', 'Tool/tech name is required.') });
        setIsSubmitting(false);
        return;
      }

      const { data: { session } } = await supabase.auth.getSession();
      if (!session?.user) throw new Error(t('common.mustBeLoggedInToCreateReview', 'You must be logged in to create a review'));
      ...
      if (isEditingDraft && draftId) {
        ...
        if (error) throw error;
        newReviewId = result?.id;
        if (!newReviewId) throw new Error(t('common.failedToGetNewReviewId', 'Failed to get the ID of the newly created review.'));
      }
      ...
      setErrors({ general: err.message || t('common.failedToCreateUpdateReview', 'Failed to create/update review. Please try again.') });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <form>
      <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
        ...
        <div className="flex flex-wrap justify-between gap-3 p-4">
          <div className="flex min-w-72 flex-col gap-3">
            <p className="text-[#161118] tracking-light text-[32px] font-bold leading-tight">
              {isEditingDraft ? t('common.editToolTechReview', 'Edit Tool/Tech Review') : t('common.createToolTechReview', 'Create Tool/Tech Review')}
            </p>
            <p className="text-[#7c608a] text-sm font-normal leading-normal">
              {isEditingDraft ? t('common.continueWorkingOnReview', 'Continue working on your tool/tech review') : t('common.shareYourExperience', 'Share your experience. Fill in the details below.')}
            </p>
          </div>
        </div>
        
        <div className="p-4 border-t border-[#e2dbe6] dark:border-[#2a2a3e]">
          <h2 className="text-[#161118] dark:text-[#f5f7f8] text-xl font-semibold mb-4">{t('common.coreInformation', 'Core Information')}</h2>
          <div className="mb-6">
            <label htmlFor="title" className="block text-[#7c608a] text-sm font-medium mb-2">{t('common.title', 'Title')} *</label>
            <input id="title" name="title" type="text" value={formData.title} onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))} placeholder={t('common.whatsYourReviewAbout', "What's your review about?")} className={`w-full px-4 py-3 rounded-lg border ${errors.title ? 'border-red-500' : 'border-[#e2dbe6] dark:border-[#2a2a3e]'} bg-white dark:bg-[#0f0f1a] text-[#161118] dark:text-[#f5f7f8] focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent`} />
            {errors.title && <p className="mt-1 text-red-500 text-sm">{errors.title}</p>}
          </div>
          <div className="mb-6">
            <label htmlFor="toolTechName" className="block text-[#7c608a] text-sm font-medium mb-2">{t('common.toolTechnologyName', 'Tool/Technology Name')} *</label>
            <input id="toolTechName" name="toolTechName" type="text" value={formData.toolTechName} onChange={(e) => setFormData(prev => ({ ...prev, toolTechName: e.target.value }))} placeholder={t('common.toolTechNamePlaceholder', "e.g., VS Code, Figma, React")} className={`w-full px-4 py-3 rounded-lg border ${errors.toolTechName ? 'border-red-500' : 'border-[#e2dbe6] dark:border-[#2a2a3e]'} bg-white dark:bg-[#0f0f1a] text-[#161118] dark:text-[#f5f7f8] focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent`} />
            {errors.toolTechName && <p className="mt-1 text-red-500 text-sm">{errors.toolTechName}</p>}
          </div>
          <div className="mb-6">
            <label className="block text-[#7c608a] text-sm font-medium mb-2">{t('common.overallRating', 'Overall Rating')}</label>
          ...
          <div className="mb-6">
            <label htmlFor="oneLinerPros" className="block text-[#7c608a] text-sm font-medium mb-2">{t('common.oneLinerPros', 'One-liner Pros')}</label>
            <input id="oneLinerPros" name="oneLinerPros" type="text" value={formData.oneLinerPros} onChange={(e) => setFormData(prev => ({ ...prev, oneLinerPros: e.target.value }))} placeholder={t('common.oneLinerProsPlaceholder', "What do you love most about this tool?")} className="w-full px-4 py-3 rounded-lg border border-[#e2dbe6] dark:border-[#2a2a3e] bg-white dark:bg-[#0f0f1a] text-[#161118] dark:text-[#f5f7f8] focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" />
          </div>
          <div>
            <label htmlFor="oneLinerCons" className="block text-[#7c608a] text-sm font-medium mb-2">{t('common.oneLinerCons', 'One-liner Cons')}</label>
            <input id="oneLinerCons" name="oneLinerCons" type="text" value={formData.oneLinerCons} onChange={(e) => setFormData(prev => ({ ...prev, oneLinerCons: e.target.value }))} placeholder={t('common.oneLinerConsPlaceholder', "What are the downsides?")} className="w-full px-4 py-3 rounded-lg border border-[#e2dbe6] dark:border-[#2a2a3e] bg-white dark:bg-[#0f0f1a] text-[#161118] dark:text-[#f5f7f8] focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" />
          </div>
        </div>
        ...
        <div className="p-4 border-t border-[#e2dbe6] dark:border-[#2a2a3e]">
          <h2 className="text-[#161118] dark:text-[#f5f7f8] text-xl font-semibold mb-4">{t('common.detailedReview', 'Detailed Review')}</h2>
          <p className="text-[#7c608a] text-sm mb-4">{t('common.writeDetailedReview', 'Write your detailed review of the tool or technology.')}</p>
        ...
        <div className="p-4 border-t border-[#e2dbe6] dark:border-[#2a2a3e]">
          <div className="flex flex-wrap gap-4">
            <button type="button" onClick={handleSaveDraft} className="px-6 py-3 bg-[#e2dbe6] dark:bg-[#2a2a3e] text-[#161118] dark:text-[#f5f7f8] rounded-lg font-medium hover:bg-[#d0c5d8] dark:hover:bg-[#3a3a4e] transition-colors">
              {saveDraftStatus === 'saving' ? t('common.saving', 'Saving...') : saveDraftStatus === 'saved' ? t('common.saved', 'Saved!') : t('common.saveDraft', 'Save Draft')}
            </button>
            <button type="button" onClick={handleSubmit} disabled={isSubmitting} className="px-6 py-3 bg-primary text-white rounded-lg font-medium hover:bg-primary/80 transition-colors disabled:opacity-70 disabled:cursor-not-allowed">
              {isSubmitting ? t('common.publishing', 'Publishing...') : isEditingDraft ? t('common.updateReview', 'Update Review') : t('common.publishReview', 'Publish Review')}
            </button>
          </div>
        </div>
      </div>
