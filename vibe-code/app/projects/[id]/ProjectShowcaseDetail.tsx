...
const ProjectShowcaseDetail = ({ projectId }: { projectId: string }) => {
  ...
  useEffect(() => {
    ...
    if (!projectId) {
      setError(t('projects.projectIdMissing', 'Project ID is missing.'));
      setLoading(false);
      return;
    }
  ...
  const fetchProjectDetails = async () => {
    try {
      ...
      if (error.code === 'PGRST116') {
        ...
        setError(t('projects.errorLoadingProject', 'Error loading project information.'));
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
      ...
      <div className="px-4 py-6">
        {projectImages.length > 0 ? (
          ...
          <div className="w-full h-96 bg-gray-200 dark:bg-gray-700 rounded-xl shadow-lg flex items-center justify-center">
            <span className="text-gray-500 dark:text-gray-400">{t('projects.noImageAvailable', 'No image available')}</span>
          </div>
        )}
      </div>
      ...
      <h2 className="text-[#161118] text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5 break-words">{t('projects.keyFeatures', 'Key Features')}</h2>
      <FeatureList projectId={project?.id || ''} />
      
      <h2 className="text-[#161118] text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5 break-words">{t('projects.techStackAndTools', 'Tech Stack & Tools')}</h2>
      ...
      <div className="p-4 grid grid-cols-[20%_1fr] gap-x-6">
        {project?.github_url && (
          <div className="col-span-2 grid grid-cols-subgrid border-t border-t-[#e2dbe6] py-5">
            <p className="text-[#7c608a] text-sm font-normal leading-relaxed break-words">{t('projects.githubRepository', 'GitHub Repository')}</p>
        ...
        {project?.deployment_platform && (
          <div className="col-span-2 grid grid-cols-subgrid border-t border-t-[#e2dbe6] py-5">
            <p className="text-[#7c608a] text-sm font-normal leading-relaxed break-words">{t('projects.deploymentInfo', 'Deployment Info')}</p>
      ...
      <h2 className="text-[#161118] text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5 break-words">{t('projects.authorInfo', 'Author Information')}</h2>
      <div className="p-4">
        <AuthorProfile userId={project?.user_id || ''} />
      </div>
      
      <h2 className="text-[#161118] text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5 break-words">{t('projects.relatedProjects', 'Related Projects')}</h2>
      <RelatedProjects authorId={project?.user_id || ''} currentProjectId={project?.id || ''} />
      
      <h2 className="text-[#161118] text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5 break-words">{t('projects.comments', 'Comments')}</h2>
